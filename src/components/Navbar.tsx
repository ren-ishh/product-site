...
export default function Navbar() {
  ...
  return (
    ...
    >
      ...
      {mobileMenuOpen && (
        <div className="lg:hidden absolute top-full left-0 w-full bg-white shadow-xl border-t border-gray-100 py-4 px-6 flex flex-col gap-4 animate-fade-in">
          {navLinks.map((link) => (
            <a
              key={link.name}
              href={link.href}
          ...
          <a
            href="#products"
            onClick={() => setMobileMenuOpen(false)}
            className="mt-2 bg-[#25D366] text-white text-center font-medium px-6 py-3 rounded-xl shadow-lg shadow-[#25D366]/20 flex items-center justify-center gap-2"
          >
